module OpenSSLCookbook
  ...
  module Helpers
    ...
    def gen_dhparam(key_length)
      fail ArgumentError, 'Key length must be a power of 2 greater than or equal to 1024'\
      unless key_length_valid?(key_length)
    ...
    def encrypt_rsa_key(rsa_key, key_password)
      fail TypeError, 'rsa_key must be a Ruby OpenSSL::PKey::RSA object'\
      unless rsa_key.is_a?(OpenSSL::PKey::RSA)

      fail TypeError, 'RSA key password must be a string'\
      unless key_password.is_a?(String)
